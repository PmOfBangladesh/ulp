...
def filter_combos_by_format(combos: List[str], fmt: str) -> List[str]:
    """Filter a list of 'identifier:password' strings to only those matching *fmt*.

    Args:
        combos: List of strings in ``identifier:password`` format.
        fmt: One of ``"mailpass"``, ``"userpass"``, ``"num_pass"``.

    Returns:
        Filtered list containing only strings of the requested format.
    """
    if fmt == "mailpass":
        # Keep only lines where the left part is a valid email (has @domain)
        out = []
        for line in combos:
            ident = line.split(':')[0].strip()
            # Check if the WHOLE identifier is an email (has @)
            if '@' in ident and _EMAIL_VALIDATION_RE.match(ident.lower()):
                out.append(line)
        return out

    elif fmt == "userpass":
        # Keep only usernames: alphabetic, not email, not phone, 4+ chars
        out = []
        for line in combos:
            ident = line.split(':')[0].strip()
            ...
            cleaned = re.sub(r'[\s\-\(\)\+]', '', ident)
            if cleaned.isdigit() and len(cleaned) >= 7:
                continue
            # Skip URLs
            if ident.startswith(('http://', 'https://', 'ftp://', '//')):
                continue
            # Must contain at least one letter (not pure symbols/numbers)
            if re.search(r'[a-zA-Z]', ident):
                out.append(line)
        return out

    elif fmt == "num_pass":
        # Keep only phone numbers: starts with + or digit, 7+ digits, not email
        out = []
        for line in combos:
            ident = line.split(':')[0].strip()
            if not ident:

...
from helpers.func import run_combo_search, get_file_size_str, write_result_file, log_user_extraction, notify_combo_extraction, filter_combos_by_format
...
async def cmb_format_cb(event):
    ...
    # Filter results by selected format (mailpass / userpass / num_pass)
    matched = filter_combos_by_format(matched, fmt_key)
    if not matched:
        await event.edit("**❌ No Matching Combos Found For Selected Format**")
        return
